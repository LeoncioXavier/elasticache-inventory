...
def test_list_tags_for_resource_success():
    ...
    mock_client.list_tags_for_resource.return_value = {
        "TagList": [{"Key": "Team", "Value": "DevOps"}, {"Key": "Environment", "Value": "Production"}]
    }
...
def test_list_tags_for_resource_invalid_token():
    ...
    mock_client = MagicMock()
    error = ClientError(
        error_response={"Error": {"Code": "InvalidClientTokenId"}}, operation_name="ListTagsForResource"
    )
...
def test_describe_cache_cluster_success():
    ...
    cluster_data = {"CacheClusterId": "test-cluster", "CacheNodeType": "cache.t3.micro", "CacheNodes": []}

    mock_client.describe_cache_clusters.return_value = {"CacheClusters": [cluster_data]}
...
def test_describe_cache_cluster_invalid_token():
    ...
    error = ClientError(
        error_response={"Error": {"Code": "InvalidClientTokenId"}}, operation_name="DescribeCacheClusters"
    )
...
def test_is_invalid_client_token_true_cases():
    ...
    error = ClientError(error_response={"Error": {"Code": "InvalidClientTokenId"}}, operation_name="TestOperation")
...
def test_is_invalid_client_token_false_cases():
    ...
    valid_codes = ["ResourceNotFound", "ThrottlingException", "InternalError"]

    for code in valid_codes:
        error = ClientError(error_response={"Error": {"Code": code}}, operation_name="TestOperation")
